    ...
    result = VALUE #(
      ...
      ( module = `sap.m` control = `sap.m.MessagePopover`              name = `MessagePopoverMessageHandling`       class = `z2ui5_cl_ai_app_065` path = `src/01/b08/z2ui5_cl_ai_app_065.clas.abap`
        ...
        notes = `POST-1.71: two post-1.71 members are kept for the 1:1 port: Button.ariaHasPopup (since UI5 1.84) on the MessagePopover button, and MessagePopover.groupItems (since UI5 1.73). // IMPROVISED: the` &&
                 ...
                 ` app-authored messages go through the new z2ui5.cc.MessageManager companion control (items bound to /T_MESSAGES), which reconciles them into the message manager with a target + processor. Save stands` &&
                 ` in for generateInvalidUserInput: it walks t_forms and authors an 'A mandatory field is required' message for every EMPTY Name, each targeting its own /T_FORMS/<i>/NAME row (a filled Name is left` &&
                 ` untouched) - faithful to the original forcing the required field empty and flagging exactly it, rather than a single hardcoded target. // IMPROVISED: the sample's nested JSON model (street/name,` &&
                 ...
                 ` shortcut); isPositionable. // SUBSET: 3 of the 8 mock form rows are kept (incl. John Miller's invalid ZIP 'AAA' that trips the Integer validation on render); the single employment row is kept 1:1.` &&
                 ` Checkable against ui5/sap.m/MessagePopoverMessageHandling/localService/mockdata/FormsModel.json. // NOTE: the controller's handleMessagePopoverPress (this.oMP.toggle(oEvent.getSource())) becomes the` &&
                 ` SHOW_MESSAGES event -> client->follow_up_action( cs_event-control_by_id, toggleBy ) anchored to the button's DOM ref ($event.oSource.sId), the MessagePopover carrying id=messagePopover -` &&
                 ` open-if-closed / close-if-open, same frontend-action pattern as apps 066/067. // LIVE-TEST: confirm in a running system: the Integer/email constraint bindings auto-collect their errors into the` &&
                 ` message> model (invalid ZIP 'AAA', >40 weekly hours, malformed email) and set the field valueState; the MessagePopover button shows/hides on {message>/} and lists them; and and Save flags each EMPTY` &&
                 ` Name on its own row (a filled Name stays clean) via z2ui5.cc.MessageManager. Toggle + ZIP auto-collection visually confirmed 2026-07-21; the per-empty-row Save still to re-confirm live.`
