 ...
 * a( n = `...` ) on a sap.m control against ui5/properties.json (member
 * @since, walking the parent chain) and fails on any post-1.71 member that
 * no deviation mentions — see AGENTS §5.
 *
 ...
 * from the snapshot are skipped (members without @since count as old).
 *
...
// structural-diff: a() targets the element created last)
function usedMembers(abap) {
  const out = []; // { control, attr, line }
  ...
  for (let i = 0; i < marks.length; i++) {
    ...
    for (const a of slice.matchAll(/->\s*a\(\s*n\s*=\s*`([\w.:-]+)`/g)) {
      ...
      out.push({ control, attr: a[1], line: abap.slice(0, at + a.index).split('\n').length });
    }
...
for (const mf of fs.readdirSync(META).sort()) {
  ...
  for (const { control, attr, line } of usedMembers(abap)) {
    const since = sinceOf(control, attr);
    if (since && !leq171(since) && !declared.includes(attr.toLowerCase())) {
      const key = `${control}.${attr}`;
      if (reported.has(key)) continue;
      reported.add(key);
      console.log(`ERROR ${meta.file}:${line} — ${control}.${attr} exists only since UI5 ${since} (> 1.71) and is not declared: add a POST_171 deviation naming it`);
      errors++;
    }
  }
}
