...
> original UI5 app does: a **formatter file**. `app/webapp/Formatter.js`
> is a real, served script resource, published as the `z2ui5.Formatter`
> global (and loadable via `core:require` of `z2ui5/Formatter`), first
> entries `weightState` plus re-exports of the `z2ui5.Util` date helpers -
> Util stays the stable legacy alias and can fold in fully over time. No
> ABAP API change involved.
...
The framework ships the formatter file all abap2UI5 apps share - the
counterpart of an original app's `model/formatter.js`:
...
- **One module perspective**: `z2ui5.Formatter` re-exports the `z2ui5.Util`
  date helpers, so new code references a single module; `z2ui5.Util` remains
  the unchanged legacy alias.
...
`z2ui5_cl_ai_app_401` binds the appended table's weight state with the
original parts+formatter structure via `z2ui5.Formatter.weightState`
(LIVE-TEST pending); the render-smoke harness mirrors the module's fixed
contract. Source: abap2UI5 `app/webapp/Formatter.js`, published in
`Component.js`, specs in `node/tests/formatter.spec.js`.
