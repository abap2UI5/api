...
| Custom JS formatter **functions** (`Formatter.js`, a `formatter: '.fn'` controller method) | 🔶 | if the function is in the framework's **curated `z2ui5/Formatter` module** (a served script, CSP-clean; grows via abap2UI5 PRs - implemented 2026-07-18, pr/formatter-registry): keep the original binding structure and reference `formatter: 'z2ui5.Formatter.<fn>'`. Otherwise: reproduce the logic as a `{= … }` expression binding when it fits the parser's whitelisted globals (`parseFloat`, `isNaN`, `Math`, …), or preformat in ABAP; note it. **Never via runtime-compiled JS strings** - an eval-based `register_formatter` was rejected the same day for security (needs unsafe-eval, invites building code from data). Factories returning controls (`groupHeaderFactory`, item factories) are ❌ either way | app 401 keeps the original parts+formatter binding via `z2ui5.Formatter.weightState` (LIVE-TEST pending); source-verified in abap2UI5 `app/webapp/Formatter.js` + `Component.js` |
